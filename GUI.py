...
from API import api_twelvedata
...
def not_first_launch_check():
    try:
        ...
        logging.info(
            datetime.today().strftime('%D - %H:%M:%S') 
            + ' First launch, email subscription disabled'
        )  
...
def subscription_settings_window():
    ...
    lable_password = Label( 
        subscription_window, 
        font=('Arial,18'), 
        text='Type in your email password: '
    )
    ...
    lable_SC_minmax = Label(
        subscription_window, 
        font=('Arial,18'), 
        text='Type in tracking price for Siacoin: '
    )
    ...
    lable_BC_current = Label(
        subscription_window, text='Current: '+cost
    )
    ...
    lable_BC_minmax = Label(
        ...
        text='Type in tracking price for Bitcoin: '
    )
